...
#    #normalize data sets (min-max)
   ...
   for i in range(1, 8):
        Xlist = col_varC[0:i]
        for k in range(1, 6):
             KNN_modeling(Cmodel, Metric,  "CHM-151", Xlist, k, i)
   
   for i in range(1, 8):
        Plist = col_varP[0:i]
        for k in range(1, 6):
             KNN_modeling(Pmodel, Metric,  "PHY-151", Plist, k, i)

   for i in range(1, 8):
        Elist = col_varE[0:i]
        for k in range(1, 6):
             KNN_modeling(Emodel, Metric,  "EGR-150", Elist, k, i)
...
def KNN_modeling(df, target, course, variates, k, i):
   ...
   DAC(y_train, neighbors.predict(x_train), k, course, "train", i)
   DAC(y_test, neighbors.predict(x_test), k, course, "test", i)
   return 0
